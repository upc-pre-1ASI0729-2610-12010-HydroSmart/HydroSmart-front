/**
 * Consumption Monitoring — DashboardComponent
 *
 * Pertenece a: consumption-monitoring/presentation/dashboard
 * Por qué presentation: el componente SOLO muestra datos y reacciona al usuario.
 * NO calcula nada — delega a ConsumptionMonitoringService, IncidentDetectionService
...
export class DashboardComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit(): void {
    const userId = this.authSvc.currentUser()?.id ?? 'usr-001';
    this.monitoringSvc.initialize(userId);
    this.incidentSvc.initialize(userId);
    this.savingsSvc.initialize(userId);

    // Evaluación de meta con consumo actual del dominio
