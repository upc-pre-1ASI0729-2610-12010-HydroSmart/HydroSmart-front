/**
 * Consumption Monitoring — MockConsumptionSessionRepository
 *
 * Pertenece a: consumption-monitoring/infrastructure/repositories
 * Por qué infrastructure: implementa el contrato del dominio usando datos locales
 * (JSON). Es la capa que sabe cómo persistir y recuperar aggregates.
 * Cuando exista un backend, solo se reemplaza esta clase; el dominio no cambia.
 *
 * Transforma datos planos (JSON) → objetos del dominio ricos (aggregates/entities/VOs).
 */
...
export class MockConsumptionSessionRepository extends ConsumptionSessionRepository {
  // Cache in-memory para simular persistencia entre llamadas
  ...
  private hydrateSensors(): IoTSensor[] {
    return (devicesJson as any[]).map(raw =>
      new IoTSensor(
        raw.id, raw.userId, raw.name, raw.type, raw.location,
